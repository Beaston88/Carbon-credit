...
router.post("/transaction", verifyBuyer, createTransaction);

// get all transactions for a user (either sent or received)
router.get("/transaction", getUserTransactions);

// get a specific transaction by ID
router.get("/transaction/:id", getTransactionById);

// update the transaction status (e.g., mark as SUCCESS, FAILED)
router.put("/transaction/:id", verifyAdmin, updateTransactionStatus);
